...
inputs = [
    let
        ...
        dt = tfinal / N
        mask = Dict(l => (l > 1) * (-1)^(l == 2) * l^2 * 0.5 * amplitude for l in labels)
...
for (n, kv) in enumerate(targets)
    ...
    mse_lind = mapslices(median, mses_lind; dims=1)
    mse_pauli = mapslices(median, mses_pauli; dims=1)
    std_lind = mapslices(std, mses_lind; dims=1)
    std_pauli = mapslices(std, mses_pauli; dims=1)
    ...
    quantiles = (0.25, 0.75)
    quantile_lind = mapslices(mses -> quantile(mses, quantiles), mses_lind, dims=1)
    quantile_pauli = mapslices(mses -> quantile(mses, quantiles), mses_pauli, dims=1)
    ...
    heatmap(dropdims(mse_lind; dims=1); title="$(kv[1]) lindblad", clims=(0, 1)) |> display
