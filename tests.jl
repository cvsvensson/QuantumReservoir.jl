...
## rho evolution
@time sols1 = run_reservoir_trajectories(reservoir, rho0s, tmax, ODE(DP8()); abstol, reltol, int_alg=GaussLegendre());
...
@time sols4 = run_reservoir_trajectories(reservoir, rho0s, tmax, Exponentiation(EXP_sciml()); abstol, reltol, lindbladian);
...
inte = mm1' * stack([vecrep(rho0, reservoirs) for rho0 in rho0s])
